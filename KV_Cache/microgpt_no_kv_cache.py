...
# Training (identical to KV Cache version)
num_steps = 1000
accum_steps = 4
...
# Each new token requires re-running the full sequence from position 0.
temperature = 0.5
print("\n--- inference (WITHOUT KV Cache) ---")
generated = []
all_times = []
...
for sample_idx in range(20):
    past_tokens = [BOS]  # all generated token ids so far, starts with BOS
    sample = []
    sample_times = []
    ...
    for pos_id in range(block_size):
        ...
        # Re-run the full sequence from scratch — no KV Cache
        keys = [[] for _ in range(n_layer)]
        values = [[] for _ in range(n_layer)]
...
# For no-cache: each "step" actually includes len(past_tokens) internal gpt() calls
# So total real gpt() calls = sum(1+2+...+L for each sample)
total_real_calls = 0
for t in all_times:
    L = len(t)  # number of generated steps for this sample
    total_real_calls += L * (L + 1) // 2  # 1+2+...+L

print(f"\n--- timing summary (NO KV Cache) ---")
print(f"total gpt() calls (visible): {total_calls}")
print(f"total gpt() calls (actual, including re-runs): {total_real_calls}")
