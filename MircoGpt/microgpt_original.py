...
random.shuffle(docs) #Rearrange randomly
...
class Value:
    ...
    #data - the value of node
    #grad - partial differential
    #to remember this node depends on what node
    #local partial differential (not like grad)
    ...
    #create a new node in computation graph
    ...
    def __radd__(self, other): return self + other   #to deal with "other + value"
    def __sub__(self, other): return self + (-other) # similar to the add and reverse-add
...
#create random matrix (nout * nin), in which each is VALUE
...
params = [p for mat in state_dict.values() for row in mat for p in row] # flatten params into a single list[Value] so that we could perform gradient decrease
print(f"num params: {len(params)}") # the num of parameters
